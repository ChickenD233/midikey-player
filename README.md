...
程序只吃 MIDI 文件，不认录音。以前带「从音频转 MIDI」和「转人声 / 伴奏双轨」两个入口，
v1.1.8 已移除。

为什么删：

- 转写结果不干净。泛音、残响、鼓点、说话声都会被认成音，改谱比重新找一份谱更费时间。
- 单声部素材（哼唱、独奏）勉强能用，和弦与混音基本要人工重做。
- 专用工具做得更好。真要转写，请用专门的扒谱软件。

建议做法：

1. 上网找这首歌的 MIDI。搜索「歌名 + midi」。
2. 拿到的 `.mid` / `.midi` / `.kar` / `.rmi` 都能直接打开。
3. 找不到成品谱时，用卷帘手动改一份相近的谱，或换一首有谱的歌。
...
程序带一个纯逻辑自检，覆盖键位方案、简谱换算与播放调度。它不建窗口、不注册热键、不碰按键与 MIDI 设备。
...
低优先级音的尾巴、写一份和弦 MIDI 走完整导入链路）、声部识别与简谱换算。
