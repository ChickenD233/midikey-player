...
1. 点左栏「从音频转 MIDI…」，选一个音频文件。支持 `.wav`、`.mp3`、`.m4a`、`.aac`、`.wma`、`.flac` 等，
   具体能解哪些由 Windows 自带的解码器决定。
2. 等进度走完。窗口底部状态条显示百分比。
3. 程序把转写结果写成 `.mid` 放在音频文件旁边（重名自动加序号，不覆盖已有文件），并直接载入。
4. 剩下的用法与普通 MIDI 一样：在左侧点一行作为主旋律，再播放。

说明：

- **完全离线**。不联网、不上传，用的是内置的 basic-pitch 模型（230 KB）。
- **速度**：约每秒音频 0.1 秒（16 核机器，3 分钟的歌约 13 秒）。音频越长越慢，上限 10 分钟。
- **结果不一定干净**。转录会带出泛音与残响造成的多余音，鼓点与说话声也会被当成音高。
  卷帘里可以直接改：删音用右键，移动拖音符中部。
- **单声部素材最准**（哼唱、独奏、单音旋律）。和弦能认，但密集的混音会糊。
- **转写出来的是一份新谱**。想留档改完再点「导出 MIDI…」。
...
**声部。** 左侧列表的「声部」列写明每条轨是什么：鼓、贝斯、电吉他、吉他、键盘、弦乐、和声、人声、铜管、管乐、主音、铺底、效果。认的依据是 MIDI 文件里的 GM 音色号与通道（通道 10 恒为鼓）；文件里没写才按音域与节奏推断，推断的在提示里写「按音域与节奏推断」。识别不出就写「未标注」。标牌按四类上色：节奏（鼓）、低音（贝斯）、和声（吉他 / 键盘 / 弦乐 / 和声）、旋律（旋律 / 人声 / 主音）。鼠标停在标牌上看依据与原始轨名。

**轨名。** 文件里写了说得清的轨名就照原样显示。只写「Track 1」「声道 2」这类占位名的，改显示识别出的乐器名，原始轨名在悬浮提示里。
...
「第五人格键位」与「FF14 钢琴键位」是完整半音阶：每个半音都有自己的键，不用八度键与升半音键。
「8 键半音」只有自然音键，半音靠鼠标中键补。
「Roblox 钢琴键位」的 61 条键位 = 36 个白键 + 25 个黑键：E、B 与最高的 C7 上面没有黑键，所以正好 25 条。
「FF14 钢琴键位」的 37 条键位 = 三排各 12 个半音，再加最高的一个 do。`Shift` 一条都不用按。
...
MidiKeyPlayer/Assets/nmp.onnx    # 音频转 MIDI 的模型（basic-pitch，230 KB，嵌进 exe）
MidiKeyPlayer/Audio/        # 音频转 MIDI：解码、重采样、ONNX 执行器、音符解码
...
程序带一个纯逻辑自检，覆盖键位方案、简谱换算与音频转 MIDI。它不建窗口、不注册热键、不碰按键与 MIDI 设备。
...
断言覆盖键位方案（含 FF14 钢琴键位）、和弦调度 5 条、和弦保留 10 条
（同声部和弦一个不丢、和弦里长音时值、五音和弦、长音叠和弦、合奏优先级、
低优先级音的尾巴、写一份和弦 MIDI 走完整导入链路）、音频转 MIDI 8 条
（WAV 解析、采样率与声道、采样数、立体声降混、重采样长度与能量、同采样率直通、
模型输出帧数、和弦识别）。音频那 8 条全部自造数据，不依赖外部音频文件，
代价是每次自检多跑一次模型推理（约 0.2 秒）。
...
- 音频转 MIDI：模型是 Spotify basic-pitch（Apache-2.0，230 KB、3.6 万参数），
  以嵌入资源进 exe。推理用自写的极简 ONNX 执行器（`Audio/Onnx/`，只实现该模型用到的 23 个算子），
  不引 ONNX Runtime 原生库，所以 exe 只多约 0.2 MB。音符解码是 `basic_pitch/note_creation.py` 的 C# 移植。
- 音频解码：WAV 自己解（PCM 8/16/24/32 与 float），其它格式走 Windows 自带的 Media Foundation。
  重采样与混音是自己写的加窗 sinc（`Audio/Resampler.cs`），核只算一次、之后查表。
  裁剪会把 COM 互操作关掉（`BuiltInComInteropSupport`），csproj 里已显式打开，否则 MP3 解不开。
...
The program ships with seven built-in schemes. The scheme names are the seven names in the table below.
...
"Identity V layout" and "FF14 piano layout" are full chromatic scales: every semitone has its own key, no octave keys or semitone-up keys needed.
...
"FF14 piano layout" has 37 bindings: three rows of 12 semitones plus the top do. No `Shift` at all.
...
The FF14 piano scheme is three rows of twelve keys plus one more note.
